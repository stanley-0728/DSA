...
public class CombinationSum {
  public static void main(String[] args) {
    ...
    int k= 5;
    Arrays.sort(arr);
    // System.out.println(findCombinationWithDuplicates(new ArrayList<>(), arr, 0,k));
  ...
  static List<List<Integer>> findCombination(List<Integer>p,int[]arr,int start,int k,int target){
    ...
    if(k<0 || start>=arr.length) return new ArrayList<>();
    if(k==0){
   ...
   // Combination 3  Find all valid combinations of k numbers that sum up to n such that the following conditions are true:
