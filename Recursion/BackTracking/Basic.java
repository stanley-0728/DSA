...
public class Basic {
  public static void main(String[] args) {
    ...
    // this is a board start 0,0 and end at 2,2 give me all possible ways to reach
    // such that you are allowed only to move down and right.
  ...
  static List<String> findAllPaths(int r, int c, String p, int[][] board, boolean[][] visited, int[][] path, int step) {
    ...
    if (r < board.length && c < board[0].length && board[r][c] == 0)
      return l;
    if (r < board.length && c < board[0].length && visited[r][c])
      return l;
    visited[r][c] = true;
    path[r][c] = step;
    if (r < board.length)
